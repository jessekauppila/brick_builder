...
class BuilderAgent:
    def __init__(
        ...
        self.current_strategy = config.initial_strategy
        self.placement_history: list[PlacementRecord] = []
        self.cell_history: list[Vector3] = []
        self.trace: list[BuilderTraceEvent] = []
        self.total_score = 0.0
        self.score_breakdown = {category: 0.0 for category in config.objective_weights}
    ...
    def to_runtime_dict(self):
        return {
            ...
            "allowedStrategyShifts": list(self.config.allowed_strategy_shifts),
            "symmetryMode": self.config.symmetry_mode,
            "score": round(self.total_score, 3),
            "scoreBreakdown": {
                category: round(points, 3)
                for category, points in self.score_breakdown.items()
            },
            "buildabilityProfile": self.config.buildability_profile,
        }
    ...
    def _passes_buildability(self, analysis: PlacementAnalysis) -> bool:
        profile = self.config.buildability_profile
    ...
    def _score_candidate(
        ...
        score_breakdown: dict[str, float] = {}
        for category, weight in self.config.objective_weights.items():
    ...
    def _maybe_shift_strategy(self, world_state: WorldState, tick: int) -> None:
        available = set(self.config.allowed_strategy_shifts)
    ...
    def _commit_placement(
        ...
        if (
            self.config.symmetry_mode != "none"
            ...
            self._record_event(
                ...
                message=(
                    f"Builder {self.config.id} broke {self.config.symmetry_mode} balance."
                ),
                placement_id=placement_id,
                strategy=evaluation.strategy,
                reference_cell=evaluation.reference_cell,

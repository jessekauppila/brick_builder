...
class MatchupResult:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "scores": {builder_id: round(score, 3) for builder_id, score in self.scores.items()},
        }
...
def run_matchup_series(
    ...
    for seed in seeds:
        simulation = run_simulation(
            ...
            seed=seed,
            builders=[builder.to_dict() for builder in builder_configs],
            verbose=False,
        )
        score_map = {
            state["id"]: float(state["score"])
            for state in simulation["builderStates"]
        }
        winner_id, winner_score = max(score_map.items(), key=lambda item: item[1])
        runner_up = sorted(score_map.values(), reverse=True)[1] if len(score_map) > 1 else 0.0
        results.append(
            MatchupResult(
                seed=seed,
                builder_ids=tuple(score_map),
                winner_id=winner_id,
                score_gap=winner_score - runner_up,
                scores=score_map,
            )
        )
    return results
...
def summarize_matchup_series(results: list[MatchupResult]) -> dict[str, object]:
    if not results:
        return {"matchCount": 0, "winRates": {}, "averageScoreGap": 0.0}
